...
\author[1]{Paul Schleifer\thanks{Contact: paul_schleifer@hotmail.com. ORCID: \href{https://orcid.org/0009-0004-7972-3566}{0009-0004-7972-3566}}}
...
\begin{abstract}
Small coalitions of relatively low-influence actors have repeatedly shifted the cooperative equilibrium in climate policy negotiations. Which configurations carry disproportionate influence, and why, remains unresolved in existing game-theoretic frameworks. This paper develops a tensor-based extension of multiplayer game theory to address that gap. Standard bilateral frameworks miss dynamic influence propagation through heterogeneous networks; strategic influence is here represented as a third-order tensor $\mathcal{T} \in \mathbb{R}^{N \times N \times N}$, where each entry quantifies the joint influence of two actors on a third, and a tipping score identifies coalitions capable of shifting the cooperative equilibrium.

An exact analytical expression is derived for the first-order Z-eigenvalue shift under coalition activation for the real-data influence tensor: it equals a weighted combination of internal co-ratification alignment and outward influence spread. Eigenvalue-driven phase transitions are demonstrated on small synthetic systems; the model is then applied to 175 non-G20 nations across 263 multilateral environmental agreements (1950--2017). Vulnerability scores from ND-GAIN and WRI Aqueduct 4.0 are integrated into the reward function, incentivising inclusion of frontline-affected nations in high-scoring coalitions. A REINFORCE policy gradient agent using Gumbel top-$K$ sampling discovers coalitions that outperform exhaustive random search by 14.7\%, achieving a peak tipping score of 22,229 across 4,964 unique configurations. Keystone country analysis identifies Uruguay, Tunisia, Greece, and Algeria as consistently high-scoring anchors. All top-scoring coalitions span at least 6 of 8 treaty-participation clusters, consistent with the theoretical expectation of geographically and institutionally disparate configurations.
\end{abstract}
...
Climate change negotiations involve numerous heterogeneous actors --- from nation-states to regional coalitions and private sector alliances. Despite large-scale asymmetries in power and emissions, small coalitions have repeatedly shifted global momentum in ways that aggregate power metrics would not predict and that bilateral influence measures leave largely unexplained. The Alliance of Small Island States, representing under 1\% of global emissions, helped anchor the 1.5\textdegree C temperature goal in the Paris Agreement. The High Ambition Coalition catalysed ambitious transparency language across a far larger set of parties~\cite{yamin2021hac}. Which small coalitions carry disproportionate influence, and why, remains unresolved.

Standard cooperative game theory identifies stable allocations and pivotal players, but is not designed to model dynamic influence propagation through a heterogeneous network. Non-cooperative Nash equilibrium models assume symmetric information and discrete action spaces ill-suited to the continuous, norm-driven adjustment of multilateral diplomacy. The interaction structure of climate negotiations is triadic: each country's strategic update depends not on bilateral relationships alone, but on how \emph{pairs} of other actors jointly shape the normative environment --- a triadic structure that third-order tensors represent naturally.

Section~\ref{sec:background} provides empirical motivation and Section~\ref{sec:related} reviews related work. Section~\ref{sec:math} then formalises the third-order influence tensor and derives analytical conditions for coalition-induced tipping, including the Z-eigenvalue sensitivity result of Section~\ref{sec:tipping_score_proxy}. Simulations in Section~\ref{sec:simulation} test the model on small synthetic systems and introduce the greedy search and reinforcement learning components. Section~\ref{sec:clustering} presents a hierarchical clustering analysis of 175 non-G20 nations across 263 multilateral treaties, identifying eight treaty-participation communities that motivate cross-cluster coalition search. The real-data pipeline of Section~\ref{sec:results_realdata} integrates treaty participation, ND-GAIN vulnerability, and WRI Aqueduct water stress data, and reports results from a REINFORCE policy gradient agent using Gumbel top-$K$ sampling that achieves a 14.7\% improvement over random search without the reward sparsity problems of Bernoulli-based approaches. Section~\ref{sec:discussion} discusses implications and limitations, and Section~\ref{sec:conclusion} concludes.
...
The Alliance of Small Island States (AOSIS) is perhaps the most cited example. Despite representing fewer than 1\% of global greenhouse gas emissions, AOSIS was instrumental in establishing the 1.5\textdegree C aspirational target in the Paris Agreement, a position far from settled at the outset of COP21~\cite{aosis2023, betzold2012aosis}. The High Ambition Coalition, a cross-regional grouping of approximately 100 parties, emerged during COP21 to push for ambitious language on long-term temperature goals and transparency mechanisms. Both cases turn on alignment, geographic diversity, and strategic timing --- not aggregate emissions or economic power.

Which properties, then, characterise small coalitions with disproportionate influence across the multi-dimensional strategic landscape of climate negotiations? Standard approaches from cooperative game theory, such as Shapley values and core analysis, address payoff allocation but are not designed to model the dynamic propagation of influence through a heterogeneous network of actors. Non-cooperative approaches based on Nash equilibrium assume symmetric information and well-defined action spaces that do not naturally accommodate the continuous, norm-driven adjustment observed in multilateral diplomacy~\cite{barrett1994self, carraro1993strategies, nordhaus2015climate}.

The interaction structure in climate negotiations is triadic: a country's strategic update depends not only on bilateral relationships but on how \emph{pairs} of other actors jointly influence the normative landscape, a structure that third-order tensors represent directly. Representing influence as a tensor $\mathcal{T} \in \mathbb{R}^{N \times N \times N}$ makes nonlinear coalition dynamics tractable in ways bilateral frameworks cannot. A reinforcement learning policy gradient optimiser trained on real treaty participation data then identifies candidate tipping coalitions across the full space of non-G20 nations.

\section{Related Work}
\label{sec:related}

Climate change negotiations have long been studied through the lens of game theory, particularly in the context of public goods and cooperative behaviour among sovereign actors. Early work in this field focused on repeated games and Nash equilibrium approaches, assuming rational actors with symmetric incentives \citep{barrett1994self, carraro1993strategies, nordhaus2015climate, weischer2012climate}. Coalition stability in the IEA context is analysed at length in \citet{barrett2003environment} and \citet{finus2001game}; for a review of formal models of climate cooperation, see \citet{hovi2014hope}. However, these frameworks often fall short when modelling heterogeneous influence, asymmetric stakes, and dynamic tipping behaviours.
...
Roemer’s \textit{Kantian equilibrium} framework \citep{roemer2010kantian} also shifts the focus from traditional utility-maximising behaviour to principled cooperative norms, offering a route to understand policy emergence when actors care about global outcomes. \citet{young2015individual} examines social norms, tipping points, and behavioural diffusion as mechanisms for large-scale coordination, offering a complementary perspective on norm emergence in collective-action settings.

On the computational side, tensor methods have seen increasing application in multi-agent systems and generalised games \citep{kolda2009tensor}, but have yet to be widely adopted in the domain of environmental policy modelling. Tensor representations have not been combined with coalition tipping logic in climate negotiation analysis; this paper applies that combination to the identification of catalytic influence.
...
The present paper adds a third-order tensor layer to this literature, formally connecting spectral bifurcation conditions to a tractable coalition scoring rule and testing it on real treaty participation data, within the broader institutional context of the overlapping climate regime complex \citep{keohane2011regime}.
...
To assess the stability of equilibria, we consider the spectral properties of the tensor $\mathcal{T}$. In particular, we analyse the dominant \textit{Z-eigenvalue}~\citep{qi2005eigenvalues} $\lambda_{\max}$ of $\mathcal{T}$:
\begin{equation}
...
Two scenarios of increasing complexity test the tensor-based model: a small illustrative system and a scalable extended setup for tipping and learning hypotheses.
...
A synthetic network simulating trade or geopolitical ties yields standard centrality measures: \textit{betweenness centrality} (an approximation of broker power) and \textit{eigenvector centrality} (a recursive measure of influence proximity). Players scoring highly on these metrics are structurally positioned to propagate influence.
...
\href{https://github.com/its-not-rocket-science/tensor-based-game-theory-identifying-critical-coalitions-climate-change-negotiations/blob/main/python/greedy_tipping_coalition.py}{\texttt{greedy\_tipping\_coalition.py (GitHub)}}


\subsection{Reinforcement Learning for Coalition Formation}

We train a reinforcement learning agent using the REINFORCE algorithm~\citep{williams1992simple, sutton2018reinforcement} to learn coalition formation strategies. The policy network outputs inclusion probabilities for each nation, and coalitions are sampled from these distributions at each training step. Rewards are computed based on system-wide cooperation, tipping behaviour, and coalition size constraints.
...
\url{https://github.com/its-not-rocket-science/tensor-based-game-theory-identifying-critical-coalitions-climate-change-negotiations}
...
The IEA ratification dataset~\cite{bellelli2021iea}, covering participation in 263 multilateral treaties from 1950 to 2017, provides the basis for investigating strategic alignment among countries. A binary country--treaty matrix is constructed, and pairwise similarity between countries is computed from shared ratification profiles.

Hierarchical clustering with cosine similarity and Ward linkage partitions countries into eight primary clusters. Each cluster is labelled descriptively by its dominant UNSD sub-region (e.g., \textit{Northern Africa(6) + 4 more}), providing a high-level summary of geopolitical alignment and regional proximity.
...
\begin{quote}
	\href{https://github.com/its-not-rocket-science/tensor-based-game-theory-identifying-critical-coalitions-climate-change-negotiations/blob/main/outputs/minor_tipping_coalitions_2_to_3_members.csv.gz}{\texttt{minor\_tipping\_coalitions\_2\_to\_3\_members.csv.gz (GitHub)}}
\end{quote}
...
\begin{itemize}
	\item \textbf{High Ambition Coalition (HAC)}~\cite{yamin2021hac} — a diplomatic bloc that successfully pushed for the 1.5°C temperature goal in the Paris Agreement;
	\item \textbf{Alliance of Small Island States (AOSIS)}~\cite{aosis2023, betzold2012aosis} — a long-standing coalition of climate-vulnerable island nations;
...
\begin{table}[htbp]
	\centering
	\scriptsize
	\input{matched_real_world_coalitions_summary.tex}
	\caption{Sample of high-scoring minor-player coalitions with overlaps to known real-world initiatives. Full results available at:
		\href{https://github.com/its-not-rocket-science/tensor-based-game-theory-identifying-critical-coalitions-climate-change-negotiations/blob/main/python/outputs/matched_real_world_coalitions.csv}{\texttt{matched\_real\_world\_coalitions.csv (GitHub)}}}
...
\begin{table}[htbp]
	\centering
	{\footnotesize
		\input{rl_experiment_summary_table.tex}
	}
...
The SIDS representation in high-scoring coalitions (Bahamas, Saint Kitts and Nevis, Marshall Islands, Palau) reflects the vulnerability multiplier's effect: SIDS are assigned a Sea Level Exposure score of 1.0, the maximum possible, and carry composite vulnerability scores near 0.5 that substantially boost the reward multiplicative term. This preference arises through reward maximisation: the vulnerability multiplier creates an incentive for the agent to include high-exposure nations, without encoding their inclusion as a hard membership constraint. Coalitions that score highly under this formulation also tend to include nations most directly threatened by climate outcomes, consistent with equity-based criteria in climate governance.

\subsection*{Cross-Cluster Diversity in High-Scoring Coalitions}

All top-10 coalitions in the RL sample span at least 6 of the 8 hierarchical treaty clusters, making cross-cluster diversity a consistent empirical feature of high-scoring configurations under this reward function. This arises from the multiplicative structure of the reward: the ClusterDiversity term can only reach values of 6 or 7 if coalition members are drawn from correspondingly many distinct ratification communities. Single-cluster coalitions, regardless of internal alignment, receive a low diversity multiplier.

This pattern is consistent with the theoretical expectation in Section~\ref{sec:clustering} that high-scoring configurations draw from geographically and institutionally dispersed treaty communities. Within-cluster groupings reflect existing regional blocs and are likely already explored diplomatically. Cross-cluster alliances that bridge distant treaty communities represent the configurations that computational coalition discovery is designed to surface.

\subsection*{Comparison to Real-World Coalitions}

Several model-identified high-scoring coalitions contain members of known real-world climate initiatives (Section~\ref{sec:empirical_validation}). AOSIS-affiliated nations (Bahamas, Saint Kitts and Nevis, Marshall Islands, Samoa, Vanuatu, Palau) appear frequently in top-ranking coalitions. This overlap supports the model's empirical plausibility: nations that have historically formed influential coalitions in actual negotiations also score highly under our tipping score framework. This alignment is not guaranteed by construction, since the tipping score is derived from treaty co-ratification patterns and influence spread, not from historical negotiation outcomes. The overlap thus reflects convergent plausibility: the treaty-based scoring criterion and historically observed coalition behaviour point independently to overlapping sets of countries, without the former having been calibrated against the latter.
...
The results suggest several hypotheses of potential interest to researchers studying climate coalition formation. Uruguay, Tunisia, Greece, and Algeria appear disproportionately across high-scoring configurations and merit examination as potential coalition anchors in further empirical and diplomatic-historical work. The cross-cluster diversity pattern raises the hypothesis that high-scoring configurations tend to draw from geographically and institutionally dispersed treaty communities \citep{keohane2011regime, ostrom2010polycentric}. The vulnerability weighting mechanism illustrates how equity considerations can be embedded in a scoring objective: the reward structure creates an incentive for including climate-exposed nations through the scoring function, a property that may be of independent interest when designing computational tools for coalition research.
...
The high-scoring coalitions share consistent structural features. All span 6--7 of 8 treaty-participation clusters, a pattern explained by the multiplicative structure of the reward function and consistent with the expectation that geographically dispersed configurations command broader influence reach. Uruguay and Tunisia are the most frequently recurring keystone actors, appearing in 23\% and 20\% of top-100 coalitions respectively, driven by dense treaty ratification profiles and above-median climate vulnerability scores. The vulnerability weighting mechanism creates an algorithmic incentive for including Small Island Developing States and frontline-affected nations, consistent with equity-based criteria in climate governance.

This work develops a computational approach to identifying dispersed coalition configurations that score highly on treaty-based tipping criteria, generating hypotheses for empirical and diplomatic-historical investigation. Priorities for future work include extending the data pipeline to post-2017 treaty records and COP outcome data, and enriching the influence matrix with economic and diplomatic network features. Whether high-scoring configurations identified from historical treaty data correspond to observed momentum shifts in actual negotiations is the central empirical question to be resolved. Analytical determination of the tipping threshold $\lambda_{\text{crit}}$ --- currently set exogenously at 30 --- through bifurcation analysis or calibration against historical episodes remains an open problem.

\section*{Data Availability}
All data and code generated or analysed during this study are available at the project repository (link provided upon acceptance to comply with double-anonymous review policy). The IEA International Environmental Agreements Dataset, ND-GAIN Country Index, and WRI Aqueduct 4.0 are publicly available from their respective sources as cited in the text.

\section*{Acknowledgments}
The author thanks the maintainers of the IEA International Environmental Agreements Dataset, the ND-GAIN Country Index, and WRI Aqueduct 4.0 for making their data publicly available. The author used AI-assisted writing tools during the preparation of this manuscript.


\appendix

\section*{Appendix: Final Coalitions Learned by Reinforcement Learning}
...
\section*{Appendix: Cluster Composition and Internal Similarity}
...
\begin{quote}
	\href{https://github.com/its-not-rocket-science/tensor-based-game-theory-identifying-critical-coalitions-climate-change-negotiations/blob/main/outputs/table_countries_by_cluster.csv}{\texttt{table\_countries\_by\_cluster.csv (GitHub)}}
\end{quote}
...
\section*{Appendix: Greedy Algorithm Validation}
